...
for i, testSample in enumerate(dbTest):
    trueLabel = testSample[-1]
    predictionLabel = classVotes[i].index(max(classVotes[i])) #finds index of largest number in classVotes[i]

    if int(trueLabel) == int(predictionLabel):
        correct += 1
    total += 1
